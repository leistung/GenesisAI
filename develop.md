...
├── signup/
│   └── page.tsx                  # 注册页 /signup（复用 SignInContent）
├── auth/
│   └── error/
...
| `/signin` | 否（已登录重定向） | SignInContent | 登录页 |
| `/signup` | 否（已登录重定向） | SignInContent（注册模式） | 注册页 |
...
- **已登录用户访问 `/signin` 或 `/signup` 自动重定向**到 `/dashboard`
...
└── v1/
    ...
    │   └── has-password/route.ts # 检查用户是否有密码（OAuth 用户无密码）
    ├── bookmarks/
    │   ├── route.ts              # 收藏列表 / 添加收藏
    │   └── [id]/route.ts         # 取消收藏
    └── images/
...
##### `GET /api/v1/user/has-password` — 检查用户是否有密码

| 项目 | 说明 |
|------|------|
| 认证 | 需要 |
| 成功 | `200 { hasPassword: boolean }` |
| 说明 | OAuth 用户（Google 登录）无密码 |
...
| 已登录重定向 | `/signin` 和 `/signup` 已登录用户自动跳转到 `/dashboard` |
